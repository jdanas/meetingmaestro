...
const formSchema = z.object({
    ...
    attendees: z.string().email({ message: "Invalid email format." }).array()
    .transform(emails => emails.filter(email => email !== '')),
    students: z.string().email({ message: "Invalid email format." }).array()
    .transform(emails => emails.filter(email => email !== '')),
      ...
      if (storedMeetings) {
        try {
          ...
          if (parsedMeetings.length > 0) {
              ...
              form.setValue("attendees", firstMeeting.attendees);
              form.setValue("students", firstMeeting.students);
                ...
                <FormField
                    ...
                    render={({ field }) => (
                        <FormItem>
                            ...
                            <FormDescription>
                                Enter the list of attendees, separated by commas.
                            </FormDescription>
                            <FormMessage />
                        </FormItem>
                    )}
